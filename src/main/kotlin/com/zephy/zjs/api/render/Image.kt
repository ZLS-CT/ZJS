...
class Image(var image: BufferedImage?) {
    ...
    fun setTexture(tex: Texture?) {
        ...
        if (identifier == null) {
            identifier = Identifier.fromNamespaceAndPath(ZJS.MOD_ID, texture!!.uniqueName)
        }
    ...
    internal fun getIdOrRegister(): Identifier {
        if (identifier == null) {
            identifier = Identifier.fromNamespaceAndPath(ZJS.MOD_ID, "image${nextIdentifierIndex++}")
            texture?.let {
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun fromAsset(name: String) = Image(ImageIO.read(File(ZJS.ASSETS_FOLDER_PATH, name)))
        ...
        fun fromUrl(url: String, cachedImageName: String? = null): Image {
            ...
            val resourceFile = File(ZJS.ASSETS_FOLDER_PATH, cachedImageName)
        ...
        private fun getImageFromUrl(url: String): BufferedImage {
            val req = ZJS.makeWebRequest(url)
