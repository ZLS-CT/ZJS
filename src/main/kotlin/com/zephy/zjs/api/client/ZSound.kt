...
import com.zephy.zjs.internal.mixins.AbstractSoundInstanceAccessor
import com.zephy.zjs.internal.mixins.sound.SoundAccessor
import com.zephy.zjs.internal.mixins.sound.SoundManagerAccessor
import com.zephy.zjs.internal.mixins.sound.SoundEngineAccessor
 ...
 */
class ZSound(private val config: NativeObject) {
    ...
    private fun bootstrap() {
        ...
        val soundFile = File(ZJS.ASSETS_FOLDER_PATH, source)
        if (soundFile.exists()) {
            isCustom = true
    ...
    private class InitialSoundData(config: NativeObject) : SoundData {
        ...
        override var x = (config.getOrDefault("x", ZPlayer.getX()) as Number).toDouble()
        override var y = (config.getOrDefault("y", ZPlayer.getY()) as Number).toDouble()
        override var z = (config.getOrDefault("z", ZPlayer.getZ()) as Number).toDouble()
    ...
    enum class Category(override val mcValue: SoundSource) : ZWrapper<SoundSource> {
    ...
    enum class CTAttenuationType(override val mcValue: SoundInstance.Attenuation) : ZWrapper<SoundInstance.Attenuation> {
    ...
    private object CTResourcePack : PackResources {
        override fun packId() = ZJS.MOD_ID
