...
import com.zephy.zjs.internal.launch.InvokeDynamicSupport
import com.zephy.zjs.internal.launch.Local
import com.zephy.zjs.internal.utils.descriptor
import com.zephy.zjs.internal.utils.descriptorString
...
internal abstract class InjectorGenerator(protected val ctx: GenerationContext, val id: Int) {
    ...
    protected val parameterDescriptors by lazy {
        signature.parameters.map {
            ...
            if (it.local?.mutable == true && !it.descriptor.descriptor()
    ...
    fun ClassAssembly.generate() {
        ...
        val methodNode = method(
            modifiers,
            "${ZJS.MOD_ID}_${type}_${nameForInjection}_${counter++}",
            returnType.toType(),
            *parameterDescriptors.map { it.toType() }.toTypedArray(),
        ) {
    ...
    private fun MethodAssembly.generateUnboxIfNecessary(descriptor: Descriptor) {
        when (descriptor) {
            ...
            else -> checkcast(descriptor.toType())
        }
    }
