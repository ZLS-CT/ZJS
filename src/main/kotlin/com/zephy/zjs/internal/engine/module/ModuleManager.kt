package com.zephy.zjs.internal.engine.module
...
object ModuleManager {
    ...
    fun setup() {
        ZJS.MODULES_FOLDER.mkdirs()

        // Get existing modules
        val installedModules = getFoldersInDir(ZJS.MODULES_FOLDER).map(::parseModule).distinctBy {
            it.name.lowercase()
        }
    ...
    fun parseModule(directory: File): Module {
        ...
        if (metadataFile.exists()) {
            try {
                metadata = ZJS.json.decodeFromString(metadataFile.readText())
            } catch (e: Exception) {
    ...
    private fun loadAssets(modules: List<Module>) {
        modules.map {
            ...
            FileUtils.copyFileToDirectory(it, ZJS.ASSETS_FOLDER)
        }
    }
